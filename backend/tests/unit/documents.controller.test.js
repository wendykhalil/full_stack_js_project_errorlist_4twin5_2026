...
describe('documents.controller', () => {
  ...
  test('listMyDocuments returns quotes and invoices for artisan', async () => {
    ...
    Devis.find.mockReturnValue({ sort: jest.fn().mockReturnThis(), lean: jest.fn().mockResolvedValue([{ _id: 'd1' }]) });
    Facture.find.mockReturnValue({ sort: jest.fn().mockReturnThis(), lean: jest.fn().mockResolvedValue([{ _id: 'f1' }]) });
  ...
  test('createQuote creates a quote from project id', async () => {
    ...
    Project.findById.mockReturnValue({ lean: jest.fn().mockResolvedValue({ _id: 'p1', artisanId: 'artisan-1' }) });
