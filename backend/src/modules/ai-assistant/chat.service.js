...
async function fetchContext(userId, role, intents, message) {
  ...
  try {
    ...
    if (intents.includes('products')) {
      ...
      const stopWords = new Set(['les','des','du','de','la','le','un','une','pour','avec','dans','sur','quels','quel','quelle','sont','disponibles','disponible','produits','produit','matériaux','matériau','cherche','trouve','montre','voir','prix','stock']);
      const keywords = msg.replace(/[^\w\sàâäéèêëîïôùûüç]/gi, '').split(/\s+/)
        .filter(w => w.length > 2 && !stopWords.has(w)).slice(0, 4);
      ...
      const products = await Product.find(filter)
        .limit(8).populate('categoryId', 'name').lean();

      // If no results with keywords, return all approved products
      const finalProducts = products.length
        ? products
        : await Product.find({ isApproved: true }).limit(8).populate('categoryId', 'name').lean();

      ctx.products = finalProducts.map(p => ({
        name: p.name,
        price: `${p.price} TND`,
        unit: p.unit,
        stock: p.stock,
    ...
    if (intents.includes('availability')) {
      ...
      if (intents.includes('artisans') && ctx.artisans?.length) {
        // Get availability for found artisans
        const artisanIds = await ArtisanProfile.find(
          ctx.artisans.length ? {} : {}
        ).limit(5).distinct('userId');
    ...
    if (intents.includes('serviceRequests')) {
      ...
      if (role === 'PRESCRIPTEUR') {
        const statusFilter = wantsDone ? ['COMPLETED']
          : wantsCancelled ? ['CANCELLED']
          : wantsOpen ? ['OPEN', 'ASSIGNED']
          : undefined; // all
        ...
        ctx.serviceRequests = requests.map(r => ({
          ...
          assignedTo: r.assignedArtisanId
            ? `${r.assignedArtisanId.firstName} ${r.assignedArtisanId.lastName}`
            : null,
          deadline: r.deadline?.toISOString().slice(0, 10),
          budget: r.budgetTND,
        }));
      } else if (role === 'ARTISAN') {
        const applied = await ServiceRequest.find({
    ...
    if (intents.includes('users')) {
      ...
      // Detect which role they're asking about
      const wantsPrescripteur = /prescripteur|prescripteurs|architecte/.test(msg);
      const wantsSupplier = /fournisseur|fournisseurs|supplier/.test(msg);
      const wantsArtisan = /artisan|artisans/.test(msg);

      const roleFilter = wantsPrescripteur ? 'PRESCRIPTEUR'
        : wantsSupplier ? 'SUPPLIER'
        : wantsArtisan ? 'ARTISAN'
        : null;
...
- Le champ "artisansTotalCount" = nombre EXACT et COMPLET dans la base de données. INTERDICTION de dire "il est possible qu'il y en ait d'autres" quand ce champ est présent. Dis exactement: "Il y a X artisans au total."
- Ne fabrique JAMAIS de données (prix, noms, dates, statuts)
- Ne dis jamais "il est possible qu'il y en ait d'autres" si le total est fourni
- Pour les questions générales sur la construction, utilise tes connaissances
- Formate les listes avec des tirets (-)
- Statuts: OPEN=Ouvert, ASSIGNED=Assigné, COMPLETED=Terminé, CANCELLED=Annulé, PENDING=En attente, DELIVERED=Livré`;
...
async function streamChat(messages, res) {
  ...
  // Use Node.js readable stream approach
