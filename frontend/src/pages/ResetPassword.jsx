...
export default function ResetPassword() {
  ...
  async function onSubmit(e) {
    ...
    if (!newPassword || newPassword.length < 6) { setError("Mot de passe >= 6 caractères."); return; }

    setLoading(true);
    try {
      const res = await resetPassword({ token, newPassword });
      setMsg(res?.message || "Mot de passe mis à jour.");
      setTimeout(() => navigate("/login"), 800);
    } catch (e2) {
      setError(e2.message || "Impossible de réinitialiser.");
    } finally {
      setLoading(false);
    }
  }
