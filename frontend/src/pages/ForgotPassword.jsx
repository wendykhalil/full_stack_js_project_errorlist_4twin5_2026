...
export default function ForgotPassword() {
  ...
  async function onSubmit(e) {
    ...
    if (!value) { setError("Veuillez entrer votre email."); return; }

    setLoading(true);
    try {
      const res = await forgotPassword({ email: value });
      setMsg(res?.message || "Si cet email existe, un lien a été envoyé.");
    } catch (e2) {
      setError(e2.message || "Impossible d'envoyer l'email.");
    } finally {
      setLoading(false);
    }
  }
